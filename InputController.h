...
extern "C" {
	...
	static inline CGPoint icFingerLocation(int finger, NSSet * touches, UIEvent* event)
	{
		...
		if((unsigned)finger == 0 || (unsigned)finger > [allTouches count])
		{
#ifdef ASSERT_DEBUG
			{
	...
	static inline bool icWasAClickGeneric(NSSet * touches, UIEvent *event, int fingers, int taps)
	{
		...
		if((unsigned)fingers != [allTouches count])
			return false;
		
		UITouch *touch = [[allTouches allObjects] objectAtIndex:0];
		
		return (unsigned)[touch phase] == (unsigned)UITouchPhaseEnded && (unsigned)[touch tapCount] == (unsigned)taps;
	}
